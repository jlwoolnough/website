...
```{r toc, echo=FALSE}
render_toc(filename = "/jess/GitHub/website/content/post/countsAcrossInterval.Rmd")
```
...
# Verifying script
## Paste all enrich files together
```{bash, eval=FALSE}
paste *.col > master.col
```
...
## Read file into R
```{r}
dat <- read.table("/home/jwoolnough/jess/tf_h64/master.col", sep="\t",header=TRUE)
```
...
corrplot(M, method="circle", order="hclust", tl.cex=0.6)

```
## Sample Key
```{r,comment=NA}
id <- as.data.frame(colnames(dat))
names(id) <- c("Sample Key")
